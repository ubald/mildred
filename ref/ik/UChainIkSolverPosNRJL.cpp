...
#include "UChainIkSolverPosNRJL.h"
#include <ros/ros.h>

namespace KDL
{
    UChainIkSolverPos_NR_JL::UChainIkSolverPos_NR_JL(const Chain& _chain, const JntArray& _q_min, const JntArray& _q_max, ChainFkSolverPos& _fksolver,ChainIkSolverVel& _iksolver,
    ...
    int UChainIkSolverPos_NR_JL::CartToJnt(const JntArray& q_init, const Frame& p_in, JntArray& q_out)
    {
            q_out = q_init;
            unsigned int i;
            for(i=0;i<maxiter;i++){
    ...
    UChainIkSolverPos_NR_JL::~UChainIkSolverPos_NR_JL()
    {
    }
