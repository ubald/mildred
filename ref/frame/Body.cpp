...
namespace UWalker
{
    ...
    bool Body::setup( urdf::Model model, std::string root, std::string leg_tip_prefix )
    {
        ...
        for (unsigned int i = 0; i < LEG_COUNT; i++ )
        {
            ...
            leg[i] = new UWalker::Leg(i);

            if ( !leg[i]->setup( model, chain, root, tip_ss.str() ) )
            {
    ...
    void Body::setGait( UWalker::EGaitShape shape, UWalker::EGaitSequence sequence )
    {
        switch ( shape )
        {
            case CONTINUOUS:
            default:
                gait = new UWalker::ContinuousGait( sequence );
                break;
        }
