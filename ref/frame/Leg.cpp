...
namespace UWalker
{
    ...
    bool Leg::setup( urdf::Model model, KDL::Chain chain, std::string root, std::string tip )
    {
        ...
        boost::shared_ptr<const urdf::Link> link = model.getLink( tip );
        boost::shared_ptr<const urdf::Joint> link_joint;
        ...
        ik_solver_pos.reset( new KDL::UChainIkSolverPos_NR_JL( chain, joint_min, joint_max, *fk_solver, *ik_solver_vel, 1000, 0.001 ) );
    ...
    void Leg::setGait( UWalker::Gait * gait )
    {
    ...
    bool Leg::doIK( KDL::Vector target )
    {
        ...
        //NOTE This will actually be provided by the actuator's feedback one day...
        ...
        {
            ROS_WARN( "Leg::doIK() IK Solution not found for : %s", name.c_str() );
