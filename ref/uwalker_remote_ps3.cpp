...
int main( int argc, char **argv )
{
	...
	ros::init(argc, argv, "uwalker_remote");
	ros::NodeHandle n;
	...
	controlPublisher = n.advertise<uwalker::RemoteControl>( "uwalker/control", 1 );
...
void joyCallback( sensor_msgs::Joy joyMsg )
{
	...
	)
	{
		...
		control.position.x = joyMsg.axes[ PS3_AXIS_BUTTON_CROSS_DOWN ] + ( joyMsg.axes[ PS3_AXIS_BUTTON_CROSS_UP ] * -1 );
		control.position.y = joyMsg.axes[ PS3_AXIS_BUTTON_CROSS_LEFT ] + ( joyMsg.axes[ PS3_AXIS_BUTTON_CROSS_RIGHT ] * -1 );
		control.position.z = joyMsg.axes[ PS3_AXIS_BUTTON_REAR_LEFT_1 ] + ( joyMsg.axes[ PS3_AXIS_BUTTON_REAR_RIGHT_1 ] * -1 );
		...
		control.rotation.z = joyMsg.axes[ PS3_AXIS_BUTTON_REAR_LEFT_2 ] + ( joyMsg.axes[ PS3_AXIS_BUTTON_REAR_RIGHT_2 ] * -1 );
